...
update people set email = 'testUpdate1@gmai.com' where id = 1;

-- 3. Добавьте новую запись в таблицу
insert into people (id, first_name, last_name, email, gender, age, language)
    VALUES (51, 'Ernis', 'Jaichibekov', 'ernis@gmail.com', 'Male', 18, 'Kyrgyz');
...
select * from people order by email; -- order by email desc (z -> a)
...
--select max(language) from people where gender = 'Female';
...
select language from people where gender = 'Male' group by language;

-- 25. Отобразите записи фамилии которых содержат только 7 символов и отсортируйте их по айди
select * from people where length(last_name) = 7 order by id desc;
...
select * from people where age <= 29 and email ilike '%.com' limit 5;
select min(age) from people where email ilike '%.com' limit 5;

-- 28. Отобразите только те записи из таблицы которым есть 18 и которых почта заканчивается на .com или .uk
select * from people where age >= 18 and email like '%.com' or email ilike '%.uk';
...
select * from people where first_name ilike '%a%' or last_name ilike '%a%' and age between 56 and 20;
